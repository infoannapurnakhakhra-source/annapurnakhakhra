...
export default function ThankYouPage() {
  ...
  useEffect(() => {
    ...
    async function fetchOrder() {
      try {
        const res = await fetch(`/api/orders/thank-you?token=${token}`);
        ...
        if (typeof window.fbq !== "undefined") {
          window.fbq("track", "Purchase", {
            value: Number(data.amount),
            currency: data.currency,
            content_name: "Order #" + data.orderNumber,
          });
        }
      } catch (err) {
      ...
      <div className="min-h-screen bg-amber-50">
        <header className="relative h-64 sm:h-80 md:h-96 lg:h-[28rem] bg-cover bg-center" style={{ backgroundImage: "url('/b5.webp')" }}>
          ...
          <div className="relative z-10 flex flex-col items-center justify-center h-full text-white text-center px-4 sm:px-6">
            ...
            <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl mt-3 sm:mt-4">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8 sm:py-12 md:py-16">
          <section className="bg-white rounded-2xl shadow-2xl p-10 text-center">
            <div className="inline-flex items-center justify-center w-24 h-24 bg-[#7d4b0e] rounded-full mb-6">
              <Package className="w-14 h-14 text-white" />
            </div>

            <h2 className="text-4xl font-bold mb-6" style={{ color: "#7d4b0e" }}>
              Order #{orderId || "XXXX"}
            </h2>
           ...
           <div className="bg-gradient-to-r from-orange-50 to-amber-50 rounded-xl sm:rounded-2xl p-6 sm:p-8 md:p-10 border-l-4 sm:border-l-6 md:border-l-8 border-[#7d4b0e] shadow-inner">
                  ...
                  {paymentMethod === 'COD' && (
                    ...
                    Team Will Call You to Pay 50₹ to Confirm & Dispatch Your Order, Rest Balance Amount Pay on Delivery / Cash on Delivery. <br/>
                    Thank You.
                  </p>
                  )}
            ...
            <div className="flex justify-center gap-6 mt-10">
              <Link href="/order-history">
                <button className="bg-[#7d4b0e] text-white px-8 py-4 rounded-xl font-bold">
                  <ShoppingBag /> View Orders
                </button>
              </Link>

              <Link href="/">
                <button className="border-2 border-[#7d4b0e] px-8 py-4 rounded-xl font-bold text-[#7d4b0e]">
